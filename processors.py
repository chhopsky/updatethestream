...
def strip_team_prefix(teamname):
    return teamname.upper().lstrip("TEAM").lstrip("_").lstrip(" ").lstrip("-")
...
def determine_tricode(teamname, tricodelist):
        ...
        if len(teamname_split) > 1:
            ...
            if len(teamname_split) == 2:
                if len(teamname_split[0]) and len(teamname_split[1]):
                    if is_tricode_unique(f"{teamname_split[0][0:1]}{teamname_split[1][0:1]}".upper(), tricodelist):
                        return f"{teamname_split[0][0:1]}{teamname_split[1][0:1]}".upper()
                    if len(teamname_split[0]) > 1 and len(teamname_split[1]) > 1:
                        if is_tricode_unique(f"{teamname_split[0][0:2]}{teamname_split[1][0:1]}".upper(), tricodelist) and not teamname_split[0].upper().startswith("TEAM"):
                            return f"{teamname_split[0][0:2]}{teamname_split[1][0:1]}".upper()
                        if is_tricode_unique(f"{teamname_split[0][0:1]}{teamname_split[1][0:2]}".upper(), tricodelist) and not teamname_split[0].upper().startswith("TEAM"):
    ...
    if len(teamname) > 2:
        attempt = f"T{consonants_only(strip_team_prefix(teamname))[0:2].upper()}"
        if teamname.upper().startswith("TEAM") and is_tricode_unique(attempt, tricodelist):
    ...
    if " " in teamname[0:3] and len(teamname) > 2:
        ...
        attempt = f"{new_teamname[0:1].upper()}{consonants_only(new_teamname[2:4].upper())}"
    ...
    if len(teamname) > 2:
        if is_tricode_unique(consonants_only(teamname)[0:3].upper(), tricodelist):
            if len(f"{consonants_only(teamname)[0:3].upper()}") > 1:
                return consonants_only(teamname)[0:3].upper()
    if len(teamname) > 3:
        if is_tricode_unique(f"{teamname[0:0]}{teamname[-1:]}".upper(), tricodelist):
            return f"{teamname[0:0]}{teamname[-1:].upper()}"
        if is_tricode_unique(f"{teamname[0:0]}{teamname[-2:]}".upper(), tricodelist):
            return f"{teamname[0:0]}{teamname[-2:].upper()}"
