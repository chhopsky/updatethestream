    ...
    if " " in tn or "_" in tn or "-" in tn:
        ...
        if len(tn_s) > 1:
            ...
            if len(tn_s) == 2:
                ...
                # if the first word is two characters, try that plus the first character of the second
                if len(tn_s[0]) == 2 and len(tn_s[1]):
                    if is_tricode_unique(f"{tn_s[0][0:2]}{tn_s[1][0:1]}", tricodelist):
